...
print(f"Рабочая:      n(ξ₁) = {len(xi1_train)}, n(ξ₀) = {len(xi0_train)}")
print(f"Контрольная:  n(ξ₁) = {len(xi1_test)},  n(ξ₀) = {len(xi0_test)}")

# Вспомогательные функции

alpha = 0.05
c_alpha = 1.36  # коэффициент двухвыборочного КС при α=0.05

def ks_critical(n1, n2):
    """Критическое значение двухвыборочного критерия КС при α=0.05."""
...
D_crit_train = ks_critical(len(xi1_train), len(xi0_train))
D_crit_test = ks_critical(len(xi1_test), len(xi0_test))
print(f"\n{'='*55}")
print(f"ШАГ 1. Проверяем: различаются ли группы?")
print(f"{'='*55}")
print(f"  max|F(ξ₁) − F(ξ₀)| = {D_raw:.4f}")
print(f"  D_крит(n₁={len(xi1_train)}, n₂={len(xi0_train)}, α={alpha}) = {D_crit_train:.4f}")
print(f"  {D_raw:.4f} > {D_crit_train:.4f}  →  группы РАЗЛИЧАЮТСЯ")
...
print(f"  max|F(φ(ξ₁)) − F(ξ₀)| ≤ D_крит = {D_crit_train:.4f} ?")
print(f"{'='*55}")
print(f"  Степенная  φ(ξ₁) = â·ξ₁^b̂ :  D = {D_power:.4f}")
print(f"  Линейная   φ(ξ₁) = k̂·ξ₁   :  D = {D_linear:.4f}")
print(f"  D_крит(α={alpha})              = {D_crit_train:.4f}")
print(f"")
print(f"  Степенная: {D_power:.4f} {'≤' if D_power <= D_crit_train else '>'} {D_crit_train:.4f}  →  {'ДОПУСКАЕТСЯ' if D_power <= D_crit_train else 'НЕ ДОПУСКАЕТСЯ'}")
print(f"  Линейная:  {D_linear:.4f} {'≤' if D_linear <= D_crit_train else '>'} {D_crit_train:.4f}  →  {'ДОПУСКАЕТСЯ' if D_linear <= D_crit_train else 'НЕ ДОПУСКАЕТСЯ'}")
print(f"")
...
print(f"  D_крит(n₁={len(xi1_test)}, n₂={len(xi0_test)}, α={alpha}) = {D_crit_test:.4f}")
print(f"{'='*55}")
print(f"  Степенная: D = {D_power_test:.4f}  →  {D_power_test:.4f} {'≤' if D_power_test <= D_crit_test else '>'} {D_crit_test:.4f}  →  {'ДОПУСКАЕТСЯ' if D_power_test <= D_crit_test else 'НЕ ДОПУСКАЕТСЯ'}")
print(f"  Линейная:  D = {D_linear_test:.4f}  →  {D_linear_test:.4f} {'≤' if D_linear_test <= D_crit_test else '>'} {D_crit_test:.4f}  →  {'ДОПУСКАЕТСЯ' if D_linear_test <= D_crit_test else 'НЕ ДОПУСКАЕТСЯ'}")
...
ax.set_ylabel("F(x)")
ax.set_title(f"Исходные ФР: D = {D_raw:.4f} > D_крит = {D_crit_train:.4f}\n→ группы различаются")
ax.legend()
...
ax.set_xlabel("Glucose")
ax.set_ylabel("F(x)")
ax.set_title(f"Степенная φ = â·ξ₁^b̂: D = {D_power:.4f} {'≤' if D_power <= D_crit_train else '>'} D_крит = {D_crit_train:.4f}\n→ {'ДОПУСКАЕТСЯ' if D_power <= D_crit_train else 'не допускается'}")
ax.legend()
ax.grid(True, alpha=0.3)
...
ax.set_xlabel("Glucose")
ax.set_ylabel("F(x)")
ax.set_title(f"Линейная φ = k̂·ξ₁: D = {D_linear:.4f} {'≤' if D_linear <= D_crit_train else '>'} D_крит = {D_crit_train:.4f}\n→ {'ДОПУСКАЕТСЯ' if D_linear <= D_crit_train else 'не допускается'}")
ax.legend()
ax.grid(True, alpha=0.3)
...
ax.axhline(y=D_crit_train, color="red", ls=":", label=f"D_крит = {D_crit_train:.4f}")
ax.set_xlabel("Glucose")
ax.set_ylabel("|F(φ(ξ₁)) − F(ξ₀)|")
ax.set_title(f"max|F(φ(ξ₁)) − F(ξ₀)| ≤ D_крит = {D_crit_train:.4f}")
ax.legend()
ax.grid(True, alpha=0.3)
