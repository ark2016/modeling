...
if __name__ == "__main__":
    ...
    # кластеризация на 2 кластера (как Outcome)
    km_diab = KMeans(n_clusters=2, init="kmeans++", random_state=0)
    pred = km_diab.fit_predict(X_norm)
    # совпадение с реальными метками (или инвертированное)
    ...
    # --- Коллигация (lift): k = P(A|B) / P(A) ---
    # k = P(A|B) / P(A), где A и B — "значение < медианы" для признаков i и j
    medians = np.median(X_diab, axis=0)
    n_feat = X_diab.shape[1]
    N = X_diab.shape[0]
    K = np.zeros((n_feat, n_feat))
    for i in range(n_feat):
        A = X_diab[:, i] < medians[i]
        P_A = np.sum(A) / N
        for j in range(n_feat):
            B = X_diab[:, j] < medians[j]
            P_B = np.sum(B) / N
            P_A_and_B = np.sum(A & B) / N
            P_A_given_B = P_A_and_B / P_B if P_B > 0 else 0
            K[i, j] = P_A_given_B / P_A if P_A > 0 else 0
    print(f"\n=== Коэффициент Бернштейна k = P(A|B) / P(A) ===")
    print(f"A, B: значение < медианы для соответствующего признака")
    print(f"Признаки: {feature_names}\n")
    header_str = "\t".join([fn[:6] for fn in feature_names])
    print(f"\t{header_str}")
    for i in range(n_feat):
        row_str = "\t".join([f"{K[i,j]:.3f}" for j in range(n_feat)])
        print(f"{feature_names[i][:6]}\t{row_str}")
    with open("colligation.csv", "w") as f:
        f.write("," + ",".join(feature_names) + "\n")
        for i in range(n_feat):
            f.write(feature_names[i] + "," + ",".join([f"{K[i,j]:.3f}" for j in range(n_feat)]) + "\n")
    print("Сохранено: colligation.csv")
    print("\n=== Интерпретация (критерий независимости Бернштейна) ===")
    print("k = 1.0 — признаки независимы: P(A|B) = P(A)")
    print("k > 1.0 — положительная связь (оба выше медианы чаще, чем при независимости)")
    print("k < 1.0 — отрицательная связь (оба выше медианы реже, чем при независимости)")
    print("k = 2.0 — диагональ: P(A|A)/P(A) = 1/0.5 = 2 (полная зависимость)\n")
    print("Наиболее сильные связи:")
    pairs = []
    for i in range(n_feat):
        for j in range(n_feat):
            if i != j:
                pairs.append((abs(K[i, j] - 1.0), K[i, j], feature_names[i], feature_names[j]))
    pairs.sort(reverse=True)
    for strength, k_val, fn_i, fn_j in pairs[:10]:
        direction = "отриц." if k_val < 1 else "полож."
        print(f"  {fn_i:30s} — {fn_j:30s}: k={k_val:.3f} ({direction})")